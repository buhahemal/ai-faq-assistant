...
class QAEngine {
    constructor() {
        ...
        this.qaDataPath = path.resolve(config.data.qaDataPath);
    }
    ...
    async loadQAData() {
        try {
            ...
            // Convert to QA model instances
            this.qaPairs = qaData.qa_pairs.map(qaData => QA.fromJSON(qaData));
    ...
    async initialize() {
        try {
            ...
            this.embedder = await pipeline('feature-extraction', config.ai.model);
    ...
    async getEmbedding(text) {
        const output = await this.embedder(text, { 
            pooling: config.ai.pooling, 
            normalize: config.ai.normalize 
        });
    ...
    async saveQAData() {
        try {
            const qaData = {
                qa_pairs: this.qaPairs.map(qa => qa.toJSON()),
                metadata: {
                    version: "1.0.0",
                    total_questions: this.qaPairs.length,
                    total_answers: this.qaPairs.reduce((sum, qa) => sum + qa.answers.length, 0),
                    categories: this.getCategories(),
                    last_updated: new Date().toISOString(),
                    description: "AI FAQ Assistant Knowledge Base with support for multiple answers per question"
                }
            };

            await fs.writeFile(this.qaDataPath, JSON.stringify(qaData, null, 2));
            console.log('QA data saved successfully');
        } catch (error) {
    ...
    getQAPairs() {
        return this.qaPairs.map(qa => qa.toJSON());
    }
